...
function vjezba_6_2() {
    ...
    function draw() {
        ...
        gl.uniformMatrix2fv(u_mTrans, false, [1.0, 0.0, 0.0, 1.0]); // jedinična matrica
        gl.uniform4fv(u_boja, [0.0, 1.0, 0.0, 1.0]); // zelena boja
        gl.drawArrays(gl.TRIANGLE_FAN, 0, vertices.length / 2);
        
        let angle = Math.PI / 3.0;
        gl.uniformMatrix2fv(u_mTrans, false, [Math.cos(angle), Math.sin(angle), -Math.sin(angle), Math.cos(angle)]);
